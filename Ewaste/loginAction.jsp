...
rs=stmt.executeQuery("select username,password from customer where username='"+user+"' and password='"+pass+"'");
	while(rs.next())
	{
		un=rs.getString(1);
		 ps=rs.getString(2);
	...
	if(un.equals(user)&& ps.equals(pass))
		{
			...
			response.sendRedirect("http://localhost:8080/examples/jsp/Ewaste/logSucc.jsp");
		}
	else{
		out.println("<script type=\"text/javascript\"> ");
